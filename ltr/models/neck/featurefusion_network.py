...
class FeatureFusionNetwork(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_featurefusion_layers=4,
        ...
        self.encoder = Encoder(featurefusion_layer, num_featurefusion_layers)#将featurefusion_lauer复制了4次，

        decoderCFA_layer = DecoderCFALayer(d_model, nhead, dim_feedforward, dropout, activation)
        decoderCFA_norm = nn.LayerNorm(d_model)
        self.decoder = Decoder(decoderCFA_layer, decoderCFA_norm)#复制1次decoderCFA_layer

        self._reset_parameters()#通过网络层时，输入和输出的方差相同，包括前向传播和后向传播
    ...
    def forward(self, src_temp, mask_temp, src_search, mask_search, pos_temp, pos_search):
        src_temp_size = src_temp.shape
        src_temp = src_temp.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1)#[256,38,256]为[长*宽，breach，层数]
        pos_temp = pos_temp.unsqueeze(0).repeat(src_temp_size[0], 1, 1, 1, 1).permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1)
        src_search = src_search.flatten(2).permute(2, 0, 1)
        pos_search = pos_search.flatten(2).permute(2, 0, 1)
        mask_temp = mask_temp.squeeze(0).flatten(1).repeat(1,src_temp_size[0])#[38,256]为【breach，长*宽】
        mask_search = mask_search.flatten(1)#[38,1024]为【breach，长*宽】
        ...
        #将decoder整成template的形状
        hs = self.decoder(memory_temp, memory_search,
                          tgt_key_padding_mask=mask_temp,
                          memory_key_padding_mask=mask_search,
                          pos_enc=pos_search, pos_dec=pos_temp)
...
class Encoder(nn.Module):
    ...
    def forward(self, src1, src2,
        ...
        output1 = src1#template[256,38,256]
        output2 = src2#search[1024,38,256]

        for layer in self.layers:#循环4次
...
class FeatureFusionLayer(nn.Module):
    ...
    def forward_post(self, src1, src2,
                     ...
                     pos_src1: Optional[Tensor] = None,
                     pos_src2: Optional[Tensor] = None):
        q1 = k1 = self.with_pos_embed(src1, pos_src1)#将src1和pos_src1加起来
        ...
        src22 = self.self_attn2(q2, k2, value=src2, attn_mask=src2_mask,#houyueen self.self_attn2
        ...
        try:
            ...
            a=1
        src22 = self.multihead_attn2(query=self.with_pos_embed(src2, pos_src2),#houyueen self.multihead_attn2

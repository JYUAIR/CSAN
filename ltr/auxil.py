...
def loadData(name, num_components=None, half=True):
   #data_path = os.path.join(os.getcwd(),'data')
   data_path = '/home/hye/hyper/hyper_templates_git/dataset'
   if name == 'IP':
...
class HyperData(Dataset):
   def __init__(self, dataset, class_num):
      ...
      self.kind_num = len(list(set(self.labels)))
      self.label_kind = []
      self.data_kind = []
      for i in range(self.kind_num):
         label_index_tmp = (np.where(np.array(self.labels)==i))[0]
         self.data_kind.append(self.data[label_index_tmp])
         self.label_kind.append(self.labels[label_index_tmp])

   def __getitem__(self, index):
      usesame = True
      random_index = np.random.randint(0, self.class_num, self.kind_num)#每个种类中，随机选一个样本
      random_kind_index = np.arange(self.kind_num) #随机选择一个种类
      # np.random.shuffle(random_kind_index) #打乱
      label_template = []
      # img_template = []
      for i, kind in enumerate(random_kind_index):
         #随机选一个种类，每个种类随机拿一个样本和标签
         if i == 0:
            img_template = torch.from_numpy(np.asarray(self.data_kind[kind][random_index[kind]]))
            img_template = img_template.unsqueeze(0)
            label_template = torch.from_numpy(self.label_kind[kind])[random_index[kind]].unsqueeze(0)

         else:
            img_template_tmp = torch.from_numpy(np.asarray(self.data_kind[kind][random_index[kind]]))
            img_template_tmp = img_template_tmp.unsqueeze(0)
            img_template = torch.cat([img_template, img_template_tmp], dim=0)
            label_template_tmp = torch.from_numpy(self.label_kind[kind])[random_index[kind]].unsqueeze(0)
            label_template = torch.cat([label_template, label_template_tmp], dim=0)

      #获取target的特征和label
      label_target = self.labels[index].item()
      selected_smaple_target_idx = label_target * self.class_num + random_index[label_target]
      if usesame == False:
         if selected_smaple_target_idx == index: # 去除模板和样本相同的
            add_idx = np.random.randint(1, self.class_num, 1).item()
            if index % self.class_num + add_idx > self.class_num - 1:
               add_idx = add_idx - self.class_num
            index = index + add_idx
      img_target = torch.from_numpy(np.asarray(self.data[index, :, :, :]))

      return img_template, img_target, label_template, label_target
...
class HyperData_test(Dataset):
   ...
   def __getitem__(self, index):
      img_template = torch.from_numpy(np.asarray(self.data[index, :, :, :]))
      label_template = self.labels[index]
      return img_template, label_template, self.idx[index], self.idy[index]
...
class HyperData_train(Dataset):
   ...
   def __getitem__(self, index):
      img_template = torch.from_numpy(np.asarray(self.data[index, :, :, :]))
      label_template = self.labels[index]
      return img_template, label_template
...
def load_hyper(args):
   ...
   pixels,labels,idx,idy = createImageCubes(data, labels, patchsize=args.patchsize, removeZeroLabels=True,ridx=True)
   bands = pixels.shape[-1]; numberofclass = len(np.unique(labels))
   shape = pixels.shape[0:2];
   if args.tr_percent < 1: # split by percent
      x_train, x_test, y_train, y_test, idx_te, idy_te = split_data(pixels, labels,args.tr_percent, idx,idy,rand_state=args.rand_state)
   else: # split by samples per class
      x_train, x_test, y_train, y_test, idx_te, idy_te = split_data_fix(pixels, labels, args.tr_percent, idx,idy,rand_state=args.rand_state)
   if args.use_val: 
      if args.val_percent<1:
         x_val, x_test, y_val, y_test, idx_te, idy_te = split_data(x_test, y_test, args.val_percent,rand_state=args.rand_state)
      else:
         x_val, x_test, y_val, y_test, idx_te, idy_te = split_data_fix(x_test, y_test, args.val_percent,rand_state=args.rand_state)
   del pixels, labels

   train_hyper = HyperData((np.transpose(x_train, (0, 3, 1, 2)).astype("float32"), y_train), args.tr_percent)
   test_hyper = None
   if args.use_test: test_hyper = HyperData_test((np.transpose(x_test, (0, 3, 1, 2)).astype("float32"), y_test, idx_te, idy_te))
   ...
   train_loader = torch.utils.data.DataLoader(train_hyper, batch_size=args.tr_bsize, shuffle=True, drop_last=True,
                                              **kwargs)
   if args.is_source:
      train_loader.name = "train_source"
   else:
      train_loader.name = "train_target"


   if train_loader.name == "train_target":
      train_loader_for_test = torch.utils.data.DataLoader(train_hyper, batch_size=args.tr_bsize, shuffle=False, drop_last=True, **kwargs)
   train_loader.epoch_interval = 1
   train_loader.training = True
   train_loader.stack_dim = 0
   if args.use_test:
      # if args.te_bsize > len(x_train):
      #    args.te_bsize = len(x_train)
      test_loader = torch.utils.data.DataLoader(test_hyper, batch_size=args.tr_bsize, shuffle=True, drop_last=True, **kwargs)
      if args.is_source:
         test_loader.name = "test_source"
      else:
         test_loader.name = "test_target"
      test_loader.epoch_interval = 1
      test_loader.training = False
      test_loader.stack_dim = 0
   else:
      test_loader = None
   ...
   if args.use_test:       
      ...
      val_loader.name = "val"
      val_loader.epoch_interval = 1
      val_loader.training = False
      val_loader.stack_dim = 0
   else:
      val_loader  = None
   if train_loader.name == "train_target":
      return train_loader, test_loader, train_loader_for_test,  val_loader, numberofclass, bands, shape
   else:
      return train_loader, test_loader, val_loader, numberofclass, bands, shape
...
def createImageCubes(X, y, patchsize=11, removeZeroLabels=True, half=False, ridx=False, step=1):
   if removeZeroLabels:

...
def loadData(name, num_components=None, half=True):
   ...
   data_path = '/home/hye/hyper/data/data_HSI'
   if name == 'IP':
...
def load_hyper(args):
   ...
   if args.tr_percent < 1: # split by percent
      x_train, x_test, y_train, y_test = split_data(pixels, labels,args.tr_percent, idx,idy,rand_state=args.rand_state)
   else: # split by samples per class
      x_train, x_test, y_train, y_test = split_data_fix(pixels, labels, args.tr_percent, idx,idy,rand_state=args.rand_state)
   if args.use_val: 
      if args.val_percent<1:
         x_val, x_test, y_val, y_test = split_data(x_test, y_test, args.val_percent,rand_state=args.rand_state)
      else:
         x_val, x_test, y_val, y_test = split_data_fix(x_test, y_test, args.val_percent,rand_state=args.rand_state)
   del pixels, labels
   train_hyper = HyperData_train((np.transpose(x_train, (0, 3, 1, 2)).astype("float32"),y_train))
   ...
   if args.use_test: 
      test_loader  = torch.utils.data.DataLoader(test_hyper, batch_size=args.te_bsize, shuffle=True, **kwargs)
   else:
      test_loader = None

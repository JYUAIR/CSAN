    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            if self.visdom is not None:
                ...
                fig_save = self.visualize(image, out['target_bbox'], segmentation)
                import os
                folder_name = seq.name
                path = "../fig_save/"
                if not os.path.exists(path + folder_name):
                    os.makedirs(path + seq.name)
                if len(str(frame_num)) == 1:
                    str_num = "0000" + str(frame_num)
                elif len(str(frame_num)) == 2:
                    str_num = "000" + str(frame_num)
                elif len(str(frame_num)) == 3:
                    str_num = "00" + str(frame_num)
                elif len(str(frame_num)) == 4:
                    str_num = "0" + str(frame_num)
                else:
                    str_num = str(frame_num)
                fig_save.savefig(path + folder_name + "/" + str_num)
                print(f"{str_num} has been saved...")
